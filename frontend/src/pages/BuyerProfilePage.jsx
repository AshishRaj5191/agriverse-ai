...
function BuyerProfilePage() {
  ...
  useEffect(() => {
    async function fetchProfile() {
      try {
        ...
        setError(err?.response?.data?.message || 'Unable to load buyer profile.');
      } finally {
        setLoading(false);
      }
    }
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      setError(err?.response?.data?.message || 'Unable to save buyer profile.');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
      ...
      <p className="mt-1 text-sm text-slate-600">Complete your buyer details and preferred purchase locations.</p>
