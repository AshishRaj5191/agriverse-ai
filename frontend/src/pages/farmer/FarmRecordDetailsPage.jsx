...
function FarmRecordDetailsPage() {
  ...
  async function fetchRecord() {
    ...
    try {
      ...
      setFormData({
        ...
        expectedHarvestDate:
          result.data.expectedHarvestDate?.slice(0, 10) || "",
  ...
  async function handleImageSelect(file) {
    ...
    try {
      ...
      setAiError(
        err.response?.data?.message || "Failed to get AI disease prediction.",
      );
    } finally {
      setAiLoading(false);
    }
  ...
  async function handleAddFertilizer() {
    if (
      !newFertilizer.name ||
      !newFertilizer.quantity ||
      !newFertilizer.dateApplied
    ) {
  ...
  async function handleSaveAiPrediction() {
    if (!aiPrediction?.top_prediction) return;

    setError("");

    try {
      const diseaseName = aiPrediction.top_prediction.class
        .replaceAll("___", " - ")
        .replaceAll("_", " ");

      await addDiseaseEntry(id, {
        disease: diseaseName,
      ...
      setError(
        err.response?.data?.message ||
          "Failed to save AI prediction to disease history.",
      );
    }
  }
  ...
  async function handleDelete() {
    if (
      !window.confirm(
        "Are you sure you want to delete this farm record? This action cannot be undone.",
      )
    )
      return;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-2 border-b border-slate-200">
        {["details", "images", "activities", "fertilizers", "diseases"].map(
          (tab) => (
            <button
              key={tab}
            ...
            <form className="space-y-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={(e) =>
                    setFormData({ ...formData, soilInfo: e.target.value })
                  }
                  className="w-full rounded-md border border-slate-300 px-3 py-2 outline-none ring-leaf focus:ring"
                  rows={3}
                />
            ...
            {aiPrediction && (
              <div className="mt-4 rounded-lg border border-green-200 bg-green-50 p-4">
                ...
                <p className="mt-2 text-sm text-green-800">
                  ...
                  {aiPrediction.top_prediction?.class?.replaceAll("_", " ")}
                </p>

                <p className="mt-1 text-sm text-green-800">
                  <strong>Confidence:</strong>{" "}
                  {aiPrediction.top_prediction?.confidence}%
                </p>

                <button
                  type="button"
                  onClick={handleSaveAiPrediction}
                  className="mt-3 rounded-md bg-green-700 px-4 py-2 text-sm font-semibold text-white hover:bg-green-800"
                >
              ...
              <div className="space-y-3">
                {record.fertilizersUsed.map((fert, idx) => (
                  ...
                  >
                    ...
                    <p className="text-xs text-slate-500">
                      {new Date(fert.dateApplied).toLocaleDateString()}
                    </p>
                  </div>
                ))}
              </div>
            ...
            <div className="space-y-3">
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setNewDisease({ ...newDisease, disease: e.target.value })
                  }
                  className="w-full rounded-md border border-slate-300 px-3 py-2 outline-none ring-leaf focus:ring"
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setNewDisease({ ...newDisease, status: e.target.value })
                  }
                  className="w-full rounded-md border border-slate-300 px-3 py-2 outline-none ring-leaf focus:ring"
                >
                  ...
                  >
                    <div className="flex items-start justify-between">
                      <div>
                        ...
                        <p className="text-xs text-slate-500">
                          {new Date(disease.dateObserved).toLocaleDateString()}
                        </p>
                        {disease.treatment && (
                          <p className="mt-1 text-sm text-slate-600">
